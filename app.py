...
def extract_domain_from_filename(filename):
    ...
    # Check for backlinks_refdomains first (more specific)
    if '-backlinks_refdomains' in name:
        domain = name.replace('-backlinks_refdomains', '')
    # Then check for regular backlinks
    elif '-backlinks' in name:
...
def merge_backlink_files(backlinks_path, refdomains_path, backlinks_filename, refdomains_filename):
    """Merge backlinks and refdomains files"""
    try:
        # Validate domain consistency first
        domain = validate_domain_consistency(backlinks_filename, refdomains_filename)
        
        # Read Excel files
        df_backlinks = pd.read_excel(backlinks_path)
        df_refdomains = pd.read_excel(refdomains_path)
...
def upload_files():
    ...
    try:
        ...
        # Merge files
        merged_df = merge_backlink_files(backlinks_path, refdomains_path, backlinks_file.filename, refdomains_file.filename)
        
        # Create output filename with domain and date
        domain = extract_domain_from_filename(backlinks_file.filename)
